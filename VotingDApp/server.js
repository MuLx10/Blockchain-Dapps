...
const candidates = ['Mehul', 'Sayan', 'Vernwal'];
...
.then((accounts) => {
  ...
  .then((result) => {
    ...
    return deployedContract.methods.totalVotesFor(asciiToHex('Mehul')).call();
  })
  .then((votesMehul) => {
    console.log('votesMehul', votesMehul);
    return deployedContract.methods.voteForCandidate(asciiToHex('Mehul')).send();
  })
  .then((voteResult) => {
